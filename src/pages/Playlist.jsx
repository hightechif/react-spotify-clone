...
const Playlist = ({ onPlaySong }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gradient-to-b from-[#3b5f7f] to-[#121212] pb-24">
      {/* Header */}
      <div className="px-8 pt-20 pb-6 flex items-end gap-6">
        ...
        <div className="flex-1">
          ...
          <h1 className="text-white text-7xl font-bold mb-6">{playlist.name}</h1>
          <p className="text-gray-300 mb-4">{playlist.description}</p>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-white font-semibold">{playlist.creator}</span>
            <span className="text-gray-300">•</span>
            <span className="text-gray-300">{playlist.followers} likes</span>
            <span className="text-gray-300">•</span>
            <span className="text-gray-300">{playlistSongs.length} songs</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 pb-6">
        <div className="grid grid-cols-[16px_1fr_1fr_80px] gap-4 px-4 py-2 border-b border-gray-800 text-gray-400 text-sm mb-2">
          <div>#</div>
          <div>TITLE</div>
          <div>ALBUM</div>
        ...
        <div className="space-y-1">
          {playlistSongs.map((song, index) => (
            <div
              ...
              className="grid grid-cols-[16px_1fr_1fr_80px] gap-4 px-4 py-3 rounded hover:bg-[#ffffff14] cursor-pointer group"
            >
